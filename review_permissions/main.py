...
def do_allow_reviews(user_id: str) -> bool:
    ...
    if (
        ...
        performed_reviews.get(owner_id, 0) >= int(paid_reviews.get(owner_id, 0))  # haven't paid for them, ignore decimal part
    ):
        return False
...
async def main() -> None:
    global account_owners, owners, review_balances, performed_reviews, paid_reviews, bans
    await start_db()
